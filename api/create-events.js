// Create multiple calendar events using user's OAuth token

// HTML response template for iOS web view
function htmlResponse(success, message, count = 0) {
...
<head>
  ...
  <title>${success ? 'Events Created' : 'Error'}</title>
  <style>
...
<body>
  <div class="card">
    ...
    <div class="details">
      ${success ? `${count} event${count > 1 ? 's have' : ' has'} been added to your Google Calendar.` : 'Please try again.'}
    </div>
  </div>
</body>
</html>`;
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!events.length) {
      if (isGet) {
        res.setHeader('Content-Type', 'text/html');
        return res.status(400).send(htmlResponse(false, 'No events provided'));
      }
      return res.status(400).json({ error: 'No events provided' });
    }

    const timezone = process.env.DEFAULT_TIMEZONE || 'America/New_York';
    const targetCalendarId = data.calendarId || 'primary';
    const createdEvents = [];
    ...
    if (isGet) {
      res.setHeader('Content-Type', 'text/html');
      return res.status(200).send(htmlResponse(true, `${createdEvents.length} event${createdEvents.length > 1 ? 's' : ''} created!`, createdEvents.length));
    }
    ...
    if (isGet) {
      res.setHeader('Content-Type', 'text/html');
      return res.status(500).send(htmlResponse(false, 'Failed to create events'));
    }
